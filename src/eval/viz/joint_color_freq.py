...
def plot_fig_2_left(color_metrics, freq_metrics, az_metrics):
    """Create the Scatter Plot (Joint Distribution)"""
    plt.figure(figsize=(8, 6))
    scatter = plt.scatter(
        color_metrics,
        freq_metrics,
        c=az_metrics,  # Color points by orientation selectivity (azimuth index)
        cmap="viridis",
        s=100,
        edgecolors="black",
        alpha=0.7,
    ...
    plt.colorbar(scatter, label="Orientation Selectivity (Azimuth Index)")
    ...
    print(f"Plot saved as standard_rf_distribution.png")
