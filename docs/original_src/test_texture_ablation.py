...
CLASS_INDICES_PATH = os.path.join(
    PROJECT_PATH, "data", "categories16_class_indices.pkl"
)
...
def test_texture_ablation(
    model_name="alexnet",
    list_models=[],
    num_ab=48,
    n_top_col_pixel=48,
    ranking_indeces=["color", "fft_freq"],
    train_version=0,
    repeat=0,
    overwrite=False,
):
    ...
    NOTE: Counts which category the model chose, and then checks whether that choice aligns with shape or texture.
    If it does not align, it is counted as "other".
    ...
    for model_file in list_models:
        ...
        if "alexnet" in model_name:
            conv1_layer = model.layers[
                4
            ]  # NOTE: first layer, because there are 4 preprocessing layers before it
        else:
        ...
        for reverse_order in [False, True]:
            for ranking_index in ranking_indeces:
                ...
                for ablation in range(num_ab + 1):
                    ...
                    conv1_layer.set_weights(
                        [modified_weights, conv1_layer.get_weights()[1]]
                    )  # NOTE: 'ablation' number of filters are removed

                    # make predictions
                    pred = model.predict(x=x_test, verbose=0)

                    # count shape-texture stats
                    count_dict = {}  # NOTE: dict to track decision counts per category
                    for cate in cate16_class_indices.keys():
                        count_dict.update(
                            {cate: [0, 0, 0]}
                        )  # NOTE: decision count for category:  [shape, other, texture]
                    ...
                    for i in range(len(y_test)):
                        ...
                        # NOTE: parse image labels ("cat-elephant" -> ["cat", "elephant"])
                        ...
                        if shape_cate == decision:  # NOTE: shape decision
                            count[0] = count[0] + 1
                        elif text_cate == decision:  # NOTE: texture decision
                            count[2] = count[2] + 1
                        else:  # NOTE: other decision
                            count[1] = count[1] + 1

                        count_dict.update({shape_cate: count})

                    res_pd[f"{ranking_index}_{model_file}_ablation_{ablation}"] = (
                        count_dict.values()
                    )
