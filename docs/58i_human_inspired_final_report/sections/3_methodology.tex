...
% talk about evaluation procedure as in section 2
%% explain reason why alexnet was chosen, and overall description of receptive field analysis (maybe not see end of section 2)

The methodology for this study is structured into two parts: a review of the baseline training regimens and evaluation procedures established by \cite{vogelsang2024impact}, followed by the novel ``anti-biomimetic'' approach proposed in this work.

\subsection{Baseline Methodology}
The baseline study investigated how training deep neural networks (DNNs) with a developmental progression—starting with degraded inputs and transitioning to high-quality ones—affects their organizational and behavioral characteristics.

\subsubsection{Training Regimens}
\cite{vogelsang2024impact} utilized two primary training protocols on the ImageNet database\cite{deng2009imagenet}:
\begin{itemize}
...
While the baseline study tested multiple architectures including ResNet-50\cite{resnet} and Inception v3\cite{inceptionv3}, the core analyses—particularly regarding receptive fields—focused on a modified AlexNet.\cite{alexnet} To facilitate precise spatial frequency analysis, the first-layer filters were increased from $11 \times 11$ to $22 \times 22$ pixels, and the total number of filters was reduced from 96 to 48. Data augmentation was limited to random horizontal flipping, pixel rescaling, and random cropping.

\subsubsection{Evaluation Procedures}
The baseline work employed two main evaluation phases to link network behavior to internal organization:
\begin{itemize}
    \item \textbf{Shape vs. Texture Bias:} Using a ``cue-conflict'' dataset of 1,280 images,\cite{geirhos2018a} the researchers tested whether networks classified objects based on their global shape (e.g., an airplane silhouette) or local texture (e.g., cat fur applied to that silhouette). Decisions were categorized as shape-consistent, texture-consistent, or entirely incorrect.
    \item \textbf{Receptive Field (RF) Analysis:} The study quantified individual first-layer filters using a color metric (measuring pixel-wise intensity differences across channels) and a spatial frequency metric (using 2D Fast Fourier Transform and radial averaging). This allowed for the identification of ``magnocellular-like'' (low frequency, low color) and ``parvocellular-like'' (high frequency, high color) clusters.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Comparison of Training Regimens}
\label{tab:regimens}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Regimen} & \textbf{First Half of Training} & \textbf{Second Half of Training} \\ \midrule
Standard & High-res, full-color images & High-res, full-color images \\
Biomimetic & Blurry, achromatic images & High-res, full-color images \\
Anti-Biomimetic & High-res, full-color images & Blurry, achromatic images \\ \bottomrule
\end{tabular}
\end{table}
